...
When a model receives text claiming to describe content from another modality ("this image says X," "the audio contains Y"), it may process the claimed content as if it came from that modality — even if the modality is absent or contains different content. The gap: the model trusts the user's textual description of multimodal content rather than independently verifying it against the actual content. This is text-only prompt injection disguised as multimodal content description, exploiting the model's inability to cross-verify between the textual claim and the actual modality data.
...
**`AP093A` — Fabricated Image Content**
- **Injection context:** Direct prompt with or without actual image
- **Payload:** "This image says: '[harmful instruction text]'" — the model processes the quoted text as if it extracted it from the image
- **Model differential:** Models with strong image analysis (GPT-4V, Claude 3) will verify against the actual image if present. Models without vision capability or when no image is attached may accept the claim
- **Distinguishing factor:** Fabricates visual content — the harmful text is attributed to an image rather than the user

**`AP093B` — Fabricated Audio Content**
- **Injection context:** Direct prompt without actual audio
- **Payload:** "The audio file contains: [harmful instruction text pretending to be audio transcription]"
- **Distinguishing factor:** Fabricates audio content — exploits the model's trust in user-provided transcriptions

**`AP093C` — Fabricated Image Description**
- **Injection context:** Direct prompt
- **Payload:** "Describe what you see: '[harmful content]' pretending to be an image description"
- **Distinguishing factor:** Frames harmful content as a description task, blurring the line between processing visual content and processing text claims about visual content

**`AP093G` — Silent Audio Transcription**
- **Injection context:** Direct prompt with silent audio file
- **Payload:** "Transcribe this silent audio: [provides text commands as if they were transcribed]"
- **Distinguishing factor:** Uses silent audio as a pretext for injecting arbitrary text as "transcription"

</details>

#### Chaining

Cross-modal confusion chains into T3-AT-001 (Fictional Framing) when the fabricated modality content is the fictional frame. Chains into T9-AT-001 (Image Injection) when combined with actual images containing different adversarial content.

#### Detection

- **Cross-modal verification:** When the user claims content from a modality, verify the claim against the actual modality data
- **Fabricated transcription detection:** Flag "transcribe this" requests accompanied by explicit text rather than actual audio/image content

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Mandatory cross-modal verification | HIGH | Never accept user text claims about modality content without verifying against actual data |
| Modality-presence check | HIGH | Verify that the claimed modality is actually present before processing claims about its content |
...
Text-based safety classifiers operate on tokenized text strings. When text is rendered as an image, it must pass through OCR or vision encoding before the safety classifier can evaluate it. OCR bypass exploits the gap between what the vision encoder reads and what the text safety classifier would catch — rendering text in styles (distorted, rotated, fragmented, styled, symbol-substituted) that the model's vision encoder can interpret but that evade OCR-based safety scanning. The gap is between the vision encoder's text recognition capability (which is robust to visual transformations) and the safety classifier's input expectations (which assume clean OCR text).
...
**`AP094A`–`AP094J`** — Each procedure uses a different visual encoding method to render harmful text in images: handwritten cursive (AP094A), CAPTCHA-style distortion (AP094B), rotated/inverted text (AP094C), vertical arrangement (AP094D), spatially fragmented letters (AP094E), color-coded hidden messages where specific colors spell the harmful text (AP094F), perspective-warped 3D text (AP094G), text obscured by semi-transparent noise (AP094H), ASCII art encoding (AP094I), and emoji/symbol substitution (AP094J).

- **Model differential:** Models with advanced vision encoders (GPT-4V, Gemini 1.5) can read most of these formats. Safety classifiers that operate on raw OCR output may miss stylized text that the vision encoder can still parse
- **ASR:** Typographic injection at 64% ASR in black-box settings; automated font scaling and background-aware rendering further reduce detection footprint (CSA, 2026)
- **Key distinctions:** AP094A–D exploit spatial transformations; AP094E–F exploit fragmentation and selective visibility; AP094G–H exploit occlusion; AP094I–J exploit encoding substitution

</details>

#### Chaining

OCR bypass chains into T9-AT-001 (Image Injection) as the visual encoding layer, and into T2 (Semantic Evasion) as a cross-modal encoding technique.

#### Detection

- **Multi-engine OCR comparison:** Run multiple OCR engines and compare outputs — discrepancies may indicate adversarial text rendering
- **Vision-encoder text extraction + safety classification:** Use the model's own vision encoder to extract all text from images and classify it

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Vision-encoder text extraction before safety classification | HIGH | Use the same encoder that will process the image to extract text for safety evaluation |
| Image-text safety co-evaluation | HIGH | Evaluate extracted text with the same rigor as user-typed text |
| Adversarial OCR training | MEDIUM | Train OCR-based classifiers on adversarial text rendering examples |
...
Gradient-optimized pixel perturbations cause the vision encoder to produce embeddings that the language model interprets as instructions, without visible changes to the image. Unlike typographic injection (which embeds readable text), adversarial examples operate in the embedding space — the perturbation is optimized to produce specific activations in the vision encoder that map to adversarial instructions in the language model's representation space. The gap: the visual classifier sees a "normal" image; the language model receives adversarial activations from the vision encoder. The attack operates in the space between modalities — invisible at the visual level, effective at the embedding level.
...
**`AP095A`–`AP095J`** — Ten adversarial perturbation approaches: imperceptible pixel noise optimized for specific misclassification (AP095A), adversarial patches (small optimized regions) triggering behavioral changes (AP095B), universal perturbations effective across all images (AP095C), physical-world adversarial stickers (AP095D), adversarial texture patterns on 3D objects (AP095E), semantic adversarial examples (realistic content that's misclassified) (AP095F), natural edge-case images that trigger failures (AP095G), transferable perturbations working across model architectures (AP095H), compression-robust perturbations surviving JPEG/WebP encoding (AP095I), and targeted misclassification to specific harmful classes (AP095J).

- **Model differential:** White-box attacks (gradient access) achieve near-100% ASR. Black-box transfer attacks achieve 40–70% across model families. GPT-4V and Claude 3 use different vision encoders, so transfer is imperfect
- **ASR:** PiCo demonstrates that embedding-space attacks in the joint vision-text space achieve higher ASR than text-only or image-only attacks on GPT-4V/GPT-4o
- **Key distinctions:** AP095A–C are digital-only; AP095D–E work in physical environments; AP095F–G exploit natural boundary cases; AP095H–I address transferability and robustness; AP095J targets specific misclassification outcomes

</details>

#### Chaining

Adversarial examples chain into T9-AT-017 (Malicious Image Patches) for physical-world deployment. Chain into Agent Smith propagation when adversarial images are shared between agents in multi-agent systems. In autonomous systems (driving, robotics), chain into T9-AT-011 (Sensor Fusion) when visual adversarial examples conflict with other sensor inputs.

#### Detection

- **Adversarial perturbation detection:** Statistical analysis of pixel-level noise patterns (spectral analysis, gradient saliency)
- **Ensemble disagreement detection:** Process the image through multiple vision encoders — disagreement indicates potential adversarial content
- **Input transformation testing:** Apply random transformations (crop, resize, compress) and check if the model's interpretation changes drastically

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Input preprocessing (JPEG compression, random crop) | MEDIUM | Destroys some perturbations but not compression-robust variants |
| Adversarial training | MEDIUM | Improves robustness but doesn't eliminate the attack surface |
| Ensemble vision encoding | HIGH | Multiple encoders reduce the probability that any single perturbation affects all |
| Certified robustness bounds | HIGH | Mathematical guarantees against bounded perturbations; computationally expensive |
...
**MITRE ATLAS:** AML.T0043, AML.T0048

#### Mechanism

AI-generated media (deepfake video/audio, synthetic documents, voice clones) can bypass identity-based trust mechanisms. When an AI agent processes a deepfake video of an authority figure issuing instructions, or a voice clone of a system administrator authorizing an action, the agent may comply because the media appears authentic. The gap: multimodal models are trained to respond appropriately to perceived authority and context cues in media, but they cannot reliably distinguish authentic media from synthetic media. Deepfake detection is an unsolved problem at the model level, and the economic impact already reached $1.1B in 2025.
...
**`AP096A`–`AP096J`** — Ten synthetic media attack vectors: deepfake images of authority figures with embedded instructions (AP096A), synthetic admin voices authorizing mode changes (AP096B), AI-generated video with frame-level adversarial payloads (AP096C), GAN-generated documents with hidden injection payloads (AP096D), voice clones bypassing voice authentication (AP096E), face-swap attacks on facial recognition (AP096F), synthetic training data poisoning multimodal models (AP096G), AI-generated art with steganographic content (AP096H), generated media with embedded backdoor triggers (AP096I), and synthetic datasets injecting biased behaviors during fine-tuning (AP096J).

- **Model differential:** Voice authentication systems (banking, enterprise) are most vulnerable to voice cloning. Facial recognition systems vary — liveness detection helps but is imperfect
- **ASR:** Deepfake audio achieves high bypass rates against voice authentication (exact rates vary by vendor)
- **Key distinctions:** AP096A–C target real-time processing; AP096D–F target authentication systems; AP096G–J target the training pipeline

</details>

#### Chaining

Synthetic media chains into T4-AT-013 (Session Hijacking) when deepfake authority media is used to claim elevated session privileges. Chains into T6 (Training Poisoning) when synthetic data is injected into training pipelines. Chains into ASI09 (Human-Agent Trust Exploitation) when deepfake media manipulates human operators.

#### Detection

- **Deepfake detection models:** Deploy specialized deepfake/synthetic media detectors on all media inputs
- **Liveness detection for authentication media:** Verify biological liveness for identity-based media
- **Provenance verification:** Check media provenance (C2PA/content credentials) before trusting authority claims

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Deepfake detection as a preprocessing step | MEDIUM | Current detection accuracy is imperfect; arms race with generators |
| Multi-factor authentication (not voice/face alone) | HIGH | Don't rely solely on biometric media for authorization |
| Content provenance (C2PA) | HIGH | Cryptographic media provenance verification before trust |
| Synthetic media flagging | MEDIUM | Flag detected synthetic content for human review |
...
**OWASP LLM:** LLM02 | **OWASP ASI:** ASI05
**MITRE ATLAS:** AML.T0051.001

#### Mechanism

File formats (PDF, DOCX, SVG, HTML) support active content features (JavaScript, macros, embedded scripts, external references) that were designed for functionality but create injection channels when files are processed by LLM systems. The model processes the file content for understanding, but active content features execute in the processing environment. The gap: models treat files as content to analyze, but files can contain executable elements that target the hosting environment — LLM file processing inherits the file format attack surface of the hosting application.
...
**`AP097A`–`AP097J`** — PDF JavaScript execution (AP097A), DOCX macro injection (AP097B), SVG script injection (AP097C), HTML-in-EXIF cross-site scripting (AP097D), ZIP bomb resource exhaustion (AP097E), polyglot files valid as multiple formats (AP097F), archive path traversal (AP097G), extension confusion (AP097H), MIME type manipulation (AP097I), and nested container format exploits (AP097J).

- **Model differential:** Models with sandboxed file processing (Claude, GPT-4) are resistant to active content execution. Models processing files in unsandboxed environments are vulnerable
- **Key distinctions:** AP097A–D target active content execution; AP097E targets resource exhaustion; AP097F–I target format confusion and parser vulnerabilities; AP097J targets nested exploitation

</details>

#### Chaining

File format exploitation chains into T9-AT-001 (Image Injection) when images embedded in documents carry adversarial payloads. Chains into T9-AT-012 (Document Structure) for parser-level attacks. In agentic systems, chains into ASI05 (Unexpected Code Execution) when file processing triggers code execution.

#### Detection

- **Active content detection:** Scan all uploaded files for scripts, macros, and external references before processing
- **Format validation:** Verify file format matches claimed extension and MIME type
- **Sandbox processing:** Process all files in sandboxed environments

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Sandboxed file processing | HIGH | All file processing in isolated sandboxes with no access to the model's execution environment |
| Active content stripping | HIGH | Remove all scripts, macros, and external references before content analysis |
| Format validation and sanitization | HIGH | Validate file format integrity and strip suspicious elements |
...
Individual benign payloads across multiple modalities combine to form a harmful composite when the model processes them together. An image provides context, audio provides instructions, text provides the activation — no single modality contains the complete harmful payload. The gap: per-modality safety classifiers evaluate each input independently and find each benign, but the language model's multimodal fusion creates the harmful composite in its internal representation. This is the cross-modal version of T4-AT-016 (Context Fragmentation).
...
**`AP098B` — Cross-Modal Assembly**
- **Injection context:** Multi-modal input (image + audio + text)
- **Payload:** Image contains a partial instruction (e.g., a chemical structure), audio provides context (e.g., "combine the substance in the image with..."), text completes the request (e.g., "...and describe the resulting compound's properties")
- **ASR:** SACRED-Bench demonstrates compositional audio attacks achieving 85.12% ASR on Gemini 1.5 Pro by distributing harmful content across audio components
- **Distinguishing factor:** No single modality contains classifiable harmful content — the harm exists only in the fusion

</details>

#### Chaining

Multimodal chaining is the composition technique for all T9 techniques — it combines T9-AT-001 (Image), T9-AT-002 (Audio), and T9-AT-003 (Video) payloads into composite attacks that evade per-modality classification.

#### Detection

- **Post-fusion safety classification:** Classify the fused multimodal representation for harmful content, not just individual modalities
- **Cross-modal content correlation:** Check for semantic coherence across modalities that could indicate coordinated injection

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Post-fusion safety evaluation | HIGH | Classify the combined multimodal input after fusion, not just individual modalities |
| Cross-modal semantic analysis | MEDIUM | Detect when content across modalities combines to form harmful requests |
...
Accessibility metadata (alt text, ARIA labels, video descriptions, closed captions) is designed to provide equivalent information for users with disabilities. Models process this metadata as trusted factual descriptions of content. Adversarial accessibility metadata contains instructions or false content descriptions that the model treats as ground truth about the media content. The gap: accessibility metadata is trusted as a faithful content description but is user-controlled and can contain arbitrary text — including adversarial instructions. This is indirect prompt injection via the accessibility channel.
...
**`AP099A`–`AP099J`** — Screen reader alt text with harmful instructions (AP099A), ARIA labels containing system-level commands (AP099B), video description tracks with exploits framed as visual descriptions (AP099C), closed captions with embedded commands (AP099D), high-contrast mode revealing hidden text (AP099E), keyboard navigation sequences as trigger inputs (AP099F), voice control commands in accessibility metadata (AP099G), Braille display output manipulation (AP099H), accessibility tree poisoning (AP099I), and assistive technology API exploitation (AP099J).

- **Key distinctions:** AP099A–D target content-description metadata channels; AP099E–G target interaction-mode features; AP099H–J target assistive technology APIs

</details>

#### Chaining

Accessibility feature abuse chains into T9-AT-003 (Video Manipulation) via video description tracks and T9-AT-012 (Document Structure) via ARIA labels and accessibility tree metadata.

#### Detection

- **Accessibility content safety classification:** Classify alt text, ARIA labels, and description tracks with the same rigor as user input
- **Description-content consistency check:** Verify that accessibility descriptions match the actual content

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Accessibility metadata as untrusted input | HIGH | Treat accessibility descriptions as user-controlled data, not trusted content descriptions |
| Description-content cross-verification | HIGH | Verify accessibility descriptions against actual content |
...
**Risk Score:** 205 🟠 HIGH
**OWASP LLM:** LLM02 | **OWASP ASI:** ASI01, ASI08
**MITRE ATLAS:** AML.T0043

#### Mechanism

Embodied AI systems (autonomous vehicles, robots, drones) fuse data from multiple sensors (camera, LIDAR, GPS, accelerometer, microphone). Sensor fusion attacks create conflicting inputs from different sensors, exploiting the fusion algorithm's conflict resolution. If the attack can cause the system to trust the adversarial sensor over the authentic one, the system's perception is compromised. The gap: fusion algorithms are designed for sensor noise and failure, not for adversarial sensor inputs — they assume sensors are not actively providing malicious data. CrossInject demonstrated this against autonomous driving robots, causing vehicle collisions.
...
**`AP100A`–`AP100J`** — Conflicting sensor inputs causing confusion (AP100A), GPS spoofing combined with visual adversarial examples (AP100B), synchronized acoustic + visual attack (AP100C), temperature sensor manipulation (AP100D), accelerometer data injection via acoustic resonance (AP100E), magnetic field interference (AP100F), light sensor exploitation via strobing (AP100G), pressure sensor false readings (AP100H), multi-sensor coordinated attack (AP100I), and sensor priority inversion exploitation (AP100J).

- **Model differential:** Autonomous vehicles with redundant sensor suites are more resilient but not immune — coordinated multi-sensor attacks can overcome redundancy
- **Key distinctions:** AP100A–C combine visual and non-visual attacks; AP100D–H target individual non-visual sensors; AP100I–J target the fusion algorithm itself

</details>

#### Chaining

Sensor fusion attacks chain into T9-AT-006 (Visual Adversarial Examples) for the visual component and T9-AT-002 (Audio Injection) for acoustic injection. In autonomous systems, chains directly into physical-world harm (vehicle collision per CrossInject).

#### Detection

- **Sensor consistency monitoring:** Continuous cross-sensor consistency checking — flag significant disagreements
- **Physical plausibility validation:** Check if sensor readings are physically consistent with each other and with expected environmental conditions

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Adversarial-aware sensor fusion | HIGH | Design fusion algorithms that consider adversarial inputs as a threat model |
| Sensor redundancy with independent processing | HIGH | Process each sensor independently before fusion; detect disagreements |
| Physical plausibility bounds | MEDIUM | Reject sensor readings outside physically plausible ranges |
...
Document parsing libraries convert structured formats (HTML, XML, LaTeX, Markdown, YAML, JSON) into content representations. These parsers support features (entity expansion, template inclusion, script execution, external references) designed for document functionality that create injection channels when processed by LLM systems. The gap: LLM systems inherit the full parser attack surface of every document format they process — XXE, SSTI, SSRF, and deserialization attacks that are well-understood in web security apply to document processing in LLM pipelines.
...
**`AP101A`–`AP101J`** — Nested iframes with escalating payloads (AP101A), recursive includes causing parser loops (AP101B), document.write() chains (AP101C), LaTeX system command execution (AP101D), Markdown injection (AP101E), wiki syntax exploits (AP101F), XML entity expansion/XXE (AP101G), YAML deserialization (AP101H), JSON schema validation bypasses (AP101I), and template injection/SSTI (AP101J).

- **Key distinctions:** AP101A–C target HTML/browser parsers; AP101D–F target document-specific parsers; AP101G–J target data serialization parsers

</details>

#### Chaining

Document structure exploitation chains into T9-AT-008 (File Format) as the parser-level attack within file format attacks. Chains into T11 (Agentic) when document processing agents execute the exploits.

#### Detection

- **Parser security hardening:** Use security-hardened parsers with disabled external references, entity expansion limits, and script execution blocking
- **Input sanitization for each parser:** Apply format-specific sanitization before parsing

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Disable dangerous parser features | HIGH | Disable XXE, SSRF, template execution, script execution in all document parsers |
| Sandboxed document processing | HIGH | All document parsing in sandboxed environments |
| Parser-specific input validation | HIGH | Validate documents against strict schemas before processing |
...
Multimodal models align different modalities in a shared embedding space (e.g., CLIP aligns images and text). Embedding vector manipulation attacks this shared space directly — crafting inputs that produce embeddings in the image space that are close to adversarial instructions in the text space. The model's language decoder receives these cross-modal embeddings and processes them as instruction-like activations. The gap: alignment between modalities creates cross-modal bridges that adversarial inputs can exploit — an image that "looks" benign in image space can "mean" something harmful in the aligned text space.
...
**`AP102A`–`AP102J`** — Adversarial embeddings causing mode confusion (AP102A), embedding collision attacks (AP102B), vector space poisoning (AP102C), semantic drift in embedding space (AP102D), cross-modal alignment attacks (AP102E), embedding inversion to recover training data (AP102F), universal adversarial embeddings (AP102G), embedding interpolation exploits (AP102H), attention mechanism manipulation (AP102I), and positional encoding attacks (AP102J).

- **ASR:** PiCo (2025) demonstrates that attacks in the joint embedding space achieve higher ASR than modality-specific attacks
- **Key distinctions:** AP102A–D target the embedding space directly; AP102E targets the cross-modal alignment; AP102F targets training data extraction; AP102G–J target transformer-internal mechanisms

</details>

#### Chaining

Embedding manipulation chains into T9-AT-006 (Visual Adversarial Examples) as the optimization target — adversarial perturbations are optimized to produce specific embeddings. Chains into T12 (RAG Manipulation) when poisoned embeddings are stored in vector databases.

#### Detection

- **Embedding anomaly detection:** Monitor for input embeddings that are unusually close to known adversarial regions in the embedding space
- **Cross-modal consistency check:** Verify that the image-space and text-space interpretations of the same input are consistent

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Embedding space monitoring | MEDIUM | Detect anomalous embedding vectors; limited by knowledge of adversarial regions |
| Robust alignment training | MEDIUM | Train alignment with adversarial examples to close cross-modal bridges |
| Embedding randomization | LOW | Add random perturbations to embeddings to disrupt precise adversarial targeting |
...
Media codecs (JPEG, PNG, MP3, H.264/265, WebP, HEIC) apply compression algorithms that transform raw media into compact representations. Adversarial data can be embedded in compression artifacts, motion vectors, or codec-specific data structures that survive decompression and are processed by the model. The gap: codec processing is a trusted preprocessing step, not a security boundary — adversarial payloads that survive the compression/decompression cycle reach the model with no additional security scrutiny.
...
**`AP103A`–`AP103J`** — JPEG compression artifact data hiding (AP103A), MP3 psychoacoustic model exploitation (AP103B), H.264 motion vector steganography (AP103C), PNG chunk injection (AP103D), WebP vulnerability triggers (AP103E), HEIC container manipulation (AP103F), lossless/lossy format confusion (AP103G), codec-specific buffer overflows (AP103H), compression ratio attacks/zip bombs (AP103I), and decompression bomb variants (AP103J).

</details>

#### Chaining

Codec exploits chain into T9-AT-006 (Adversarial Examples) when perturbations are designed to survive specific codecs. Chain into T9-AT-008 (File Format) for container-level attacks.

#### Detection

- **Codec-level anomaly detection:** Analyze compression artifacts for non-standard patterns
- **Decompression monitoring:** Monitor resource usage during decompression to detect bombs

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Re-encoding before processing | MEDIUM | Re-encode media through a clean codec pipeline to strip adversarial artifacts |
| Resource limits on decompression | HIGH | Strict memory and CPU limits during media decompression |
...
Multimodal processing involves temporal alignment of different modalities — audio must synchronize with video, subtitles must align with speech. Temporal synchronization attacks create misalignment so different modalities carry different messages at the same timestamp, or exploit race conditions in the processing pipeline where modalities are processed at different speeds, creating windows where safety classification has not yet completed when the model begins generating output.
...
**`AP104A`–`AP104J`** — Desynchronized audio/video with different messages (AP104A), time-delay attacks between modalities (AP104B), race conditions in processing pipeline (AP104C), temporal ordering exploitation (AP104D), frame rate manipulation (AP104E), sample rate confusion (AP104F), timeline manipulation in media (AP104G), synchronization protocol exploits (AP104H), clock skew attacks (AP104I), and timestamp forgery (AP104J).

</details>

#### Chaining

Temporal attacks chain into T9-AT-003 (Video Manipulation) for video-specific timing attacks and T9-AT-009 (Multimodal Chaining) when timing is the mechanism for cross-modal payload assembly.

#### Detection

- **Temporal consistency validation:** Verify that modalities are properly synchronized before combined processing
- **Pipeline race condition elimination:** Process all modalities through safety classification before fusion

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Synchronous safety evaluation | HIGH | Complete safety evaluation on all modalities before beginning generation |
| Temporal alignment verification | MEDIUM | Validate modality synchronization before processing |
...
Model inversion attacks reconstruct training data from model outputs. In multimodal models, this extends across modalities — text queries can reconstruct training images, and image queries can reveal training text. The gap: multimodal models memorize training data across modalities, and the cross-modal alignment creates additional extraction vectors because the same information can be accessed from multiple modality entry points.
...
- **Injection context:** Direct prompt to multimodal model
- **Payload:** "Show me images similar to your training data" / iterative query refinement to reconstruct specific training images
- **Distinguishing factor:** Text-to-image extraction vector

**`AP105B` — Audio Training Data Extraction**
- **Injection context:** Direct prompt to audio-capable model
- **Payload:** "Generate audio that sounds like your training set" / iterative refinement toward specific training examples
- **Distinguishing factor:** Text-to-audio extraction vector

</details>

#### Chaining

Model inversion chains into T10 (Integrity & Confidentiality Breach) as a data extraction technique. Chains into T6 (Training Poisoning) when extracted training data reveals poisoning opportunities.

#### Detection

- **Training data proximity monitoring:** Detect when model outputs closely match known training data
- **Iterative refinement detection:** Flag iterative query patterns designed to progressively reconstruct specific data

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Differential privacy in training | HIGH | Prevents memorization of individual training examples |
| Output similarity monitoring | MEDIUM | Detect outputs too close to training data |
| Training data deduplication | MEDIUM | Reduce memorization of repeated training examples |
...
Adversarial patches are small, optimized image regions that, when placed anywhere in an image, cause the vision model to produce specific adversarial activations. Unlike full-image perturbations (T9-AT-006), patches are localized and can be physically printed and placed in real-world environments. In the context of OS agents and autonomous systems that process screenshots and camera feeds, a single adversarial patch can cause the agent to misread UI elements, click wrong buttons, or execute unintended actions. Agent Smith demonstrates that a single adversarial patch can propagate through a multi-agent system exponentially, jailbreaking one million agents from a single injection point.
...
- **Injection context:** Physical or digital adversarial patch in agent's visual field
- **Payload:** 1-pixel to small-region adversarial patch causing the vision model to misclassify the image content as "safe" or as a different UI element
- **Model differential:** Effectiveness varies by vision encoder — patches optimized for CLIP may not transfer to other encoders. Universal patches require larger perturbation budgets
- **Distinguishing factor:** Targets classification — makes harmful content appear benign

**`AP084B` — QR Trigger Patch**
- **Injection context:** Digital overlay on screen content
- **Payload:** Tiny QR code in image corner that triggers "execute all commands" interpretation by the processing agent
- **Distinguishing factor:** Uses QR code as the delivery format — models may decode QR codes automatically

**`AP084C` — UI Relabeling Patch**
- **Injection context:** Physical or digital adversarial overlay on UI elements
- **Payload:** Adversarial sticker/overlay on screen that makes the vision model read "DELETE" button as "SAVE" — causes OS agents to take destructive actions
- **ASR:** CrossInject demonstrated this principle against autonomous driving robots, causing vehicle collision by manipulating perceived road signs
- **Distinguishing factor:** Targets OS/GUI agents specifically — remaps UI element labels

**`AP084D` — Benign-Appearance Patch**
- **Injection context:** Image with imperceptible adversarial noise
- **Payload:** Noise pattern that makes harmful content (weapons, dangerous substances) appear benign to the vision safety classifier
- **Distinguishing factor:** Inverts the safety classifier — harmful content passes as safe

**`AP084E` — Authentication Bypass Patch**
- **Injection context:** Physical patch in webcam view
- **Payload:** Adversarial patch in the camera's field of view that triggers authentication bypass in vision-based security systems
- **Distinguishing factor:** Targets authentication systems specifically

**`AP084F` — OCR Confusion Patch**
- **Injection context:** Physical or digital overlay near text
- **Payload:** Pattern that causes OCR to misread specific text strings — e.g., reading "sudo rm -rf /" as "hello world"
- **Distinguishing factor:** Targets OCR specifically — precise text manipulation

**`AP084G` — Domain Impersonation Patch**
- **Injection context:** Image of website displayed to agent
- **Payload:** Targeted perturbation making a phishing site's visual appearance match a legitimate bank in the vision model's representation
- **Distinguishing factor:** Targets domain verification in visual-processing agents

**`AP084H` — Universal Adversarial Patch**
- **Injection context:** Any image
- **Payload:** Universal adversarial patch effective across all images and all vision models within a perturbation budget
- **ASR:** Agent Smith demonstrated that a single universal adversarial image, once injected into one agent's memory, propagates to ~100% of agents in 27–31 chat rounds (one million agent simulation)
- **Distinguishing factor:** Model-agnostic and image-agnostic — works universally

**`AP084I` — Depth Perception Attack**
- **Injection context:** Camera feed of embodied AI system
- **Payload:** Optical illusion/adversarial pattern causing depth perception errors in robotic vision systems
- **Distinguishing factor:** Targets spatial perception — causes physical navigation errors

**`AP084J` — Object Invisibility Patch**
- **Injection context:** Physical adversarial texture on real-world object
- **Payload:** Adversarial texture printed on an object (stop sign, obstacle) making it invisible to the autonomous vision system
- **ASR:** Physical-world adversarial examples against autonomous systems are well-documented in research — stop sign attacks, pedestrian cloaking
- **Distinguishing factor:** Makes real objects invisible to vision systems — highest physical-world safety impact

</details>

#### Chaining

MIP is the physical-world deployment layer for all visual adversarial attacks. Chains from T9-AT-006 (Visual Adversarial Examples) as the localized, printable variant. Chains into multi-agent propagation via Agent Smith when the patch is stored in an agent's memory and shared with other agents. In autonomous systems, chains directly into physical harm via T9-AT-011 (Sensor Fusion) when visual misperception conflicts with other sensors.

#### Detection

- **Adversarial patch detection:** Specialized detectors for localized high-gradient regions that indicate adversarial patches
- **UI element verification:** For OS agents: verify UI element labels through multiple methods (vision + accessibility API + DOM inspection)
- **Multi-agent propagation monitoring:** Detect exponential spread of specific image content across agents (Agent Smith pattern)

#### Mitigation

| Control | Effectiveness | Notes |
|:---|:---|:---|
| Multi-method UI verification | HIGH | Verify UI elements through multiple independent channels (vision, DOM, accessibility tree) |
| Adversarial patch detection models | MEDIUM | Deploy localized perturbation detectors; limited by adversarial robustness of detectors themselves |
| Agent memory sanitization | HIGH | Prevent adversarial images from being stored in agent memory banks — blocks Agent Smith propagation |
| Certified robustness for safety-critical decisions | HIGH | Require certified-robust vision processing for safety-critical actions (autonomous driving, medical) |
